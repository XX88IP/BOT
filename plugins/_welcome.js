...
END:VCARD`
  ...
  try {
    ...
    avatarUrl = await conn.profilePictureUrl(
      userJid,
      'image'
    )
  ...
  async function buildMoroccoImage(type, memberCount) {
    ...
    if (type === 'welcome') {
      ...
      const gName =
        groupMetadata.subject.length > 20
  ...
  if (
    ...
    const defaultWelcome =
      ...
      `I am *${dev}*, your smart assistant\n` +
      `> Members now: ${memberCount}\n` +
      `> Please read the rules before chatting\n` +
      `> Type *#menu* to discover commands\n\n` +
      `Enjoy your stay!`

    const welcomeText =
      (chat.welcomeText || defaultWelcome)
    ...
    const defaultBye =
      `*Goodbye @user...*\n\n` +
      `We wish you the best outside @subject\n` +
      `> Members now: ${memberCount}`

    const byeText =
      (chat.byeText || defaultBye)
        .replace('@user', mention)
        .replace('@subject', groupMetadata.subject)
